...
<div class="col-sm-12"><br>
    ...
    <table class="table table-bordered add-product-price-table table-condensed {{$class}}">
        ...
        @foreach($product_deatails->variations as $variation )
            ...
            @endphp
            @if($loop->first)
                @if($variation->is_foreign == 1)
                <tr>
                    <td>
                        <input type="hidden" name="single_variation_id" value="{{$variation->id}}">

                        <div class="col-sm-6">
                          {!! Form::label('single_dpp', trans('product.exc_of_tax') . '  USD:*') !!}

                          {!! Form::text('single_dpp', @num_format($variation->foreign_p_price), ['class' => 'form-control input-sm dpp input_number', 'placeholder' => __('product.exc_of_tax'), 'required']); !!}
                        </div>

                        <div class="col-sm-6">
                          {!! Form::label('single_dpp_inc_tax', trans('product.inc_of_tax') . ':*') !!}
                        
                          {!! Form::text('single_dpp_inc_tax', @num_format($variation->foreign_p_price_inc_tex), ['class' => 'form-control input-sm dpp_inc_tax input_number', 'placeholder' => __('product.inc_of_tax'), 'required']); !!}
                        </div>
                    </td>
                    ...
                    <td>
                        ...
                        {!! Form::text('single_dsp', @num_format($variation->foreign_s_price), ['class' => 'form-control input-sm dsp input_number', 'placeholder' => __('product.exc_of_tax'), 'id' => 'single_dsp', 'required']); !!}

                        {!! Form::text('single_dsp_inc_tax', @num_format($variation->foreign_s_price_inc_tex), ['class' => 'form-control input-sm hide input_number', 'placeholder' => __('product.inc_of_tax'), 'id' => 'single_dsp_inc_tax', 'required']); !!}
                    </td>
                    <td>
                        @php 
