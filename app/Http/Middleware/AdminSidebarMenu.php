...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\StockAdjustmentController::class, 'stockSurplus']),
                                'Stock Surplus',
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-surplus' && request()->segment(2) == 'stockSurplus']
                            );
                        }
            ...
            //Kitchen menu
            if (in_array('kitchen', $enabled_modules) && (auth()->user()->can('crud_all_bookings'))) {
            ...
            //Service Staff menu
            if (in_array('service_staff', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
