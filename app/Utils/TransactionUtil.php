...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'due_amount' => !empty($input['due_amount']) ? $input['due_amount'] : null,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_email == 1 && !empty($location_details->email)) {
            ...
            $output['contact'] .= '<br>' . ('business.email') . ': ' . $location_details->email;
        }
    ...
    public function getReceiptDetails1($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        $il = $invoice_layout;
        ...
        $sell_method = TransactionPayment::where('transaction_id', $transaction_id)->orderBy('created_at', 'desc')->first();
        // sell method 
        $output['sell_method'] = $sell_method->method;
        //Display name
        $output['display_name'] = $output['business_name'];
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_line_array = [
                        ...
                        'unit_price_exc_tax' => $modifier_line->unit_price,
    ...
    public function mapPurchaseSellPlus($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        if (empty($transaction_lines)) {
            return false;
        }
        ...
        foreach ($transaction_lines as $line) {
            ...
            if ($purchase_qty < 1) {
                ...
                ->where('variation_id', $line->variation_id)->orderBy('id', 'DESC')->first();
                $purchaseId = $purchase_id->id;
                // dd($purchaseId);
            }else{
                $purchase_id = $purchase_line->whereRaw("(quantity_sold+quantity_adjusted+quantity_returned+mfg_quantity_used) < quantity")
            ...
            $rows = $query->select(
                ...
                'PL.quantity_sold as quantity_sold',
                'PL.quantity_adjusted as quantity_adjusted',
                'PL.quantity_adjusted_surplus as quantity_adjusted_surplus',
                'PL.quantity_returned as quantity_returned',
                'PL.mfg_quantity_used as mfg_quantity_used',
                'transactions.invoice_no'
            )->get();
