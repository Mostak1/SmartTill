    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            ...
            if (
                ...
                string +=
                    ' (' + item.sub_sku + ')' + '<br> Price: ' + selling_price + ' (Out of stock)';
                if (item.total_quantity > 0) {
                    string +=
                        '</span></li><b> Store:' +
                        parseFloat(item.total_quantity - item.qty_available).toFixed(2) +
                        '</b><hr/>';
                } else {
                    string +=
                        '| Store:' +
                ...
                if (item.nearest_exp_date) {
                    ...
                    if (days_to_expiry <= parseInt(expiringSoon)) {
                        string = '<div style="border-left: 5px solid red;">' + string;
                    } else if (days_to_expiry <= parseInt(expiringLater)) {
