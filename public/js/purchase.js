...
function update_inline_profit_percentage(row) {
    ...
    if (row.find('input.default_sell_price').closest('td').find('.bdt-display').length) {
        var cat_desck = __read_number(row.find('input.cat_desck'), true);
        var default_purchase_price = __read_number(
            row.find('input.purchase_unit_cost_without_discount'),
            true
        );
...
function update_table_total() {
    ...
    // Update BDT value if product belongs to us category
    $('input.default_sell_price').each(function () {
        var row = $(this).closest('tr');

        // Check if the .bdt-display element exists in the closest td
        if (row.find('.bdt-display').length) {
            var cat_desck = parseFloat(row.find('input.cat_desck').val());

            // Ensure that cat_desck is a number
            if (!isNaN(cat_desck)) {
